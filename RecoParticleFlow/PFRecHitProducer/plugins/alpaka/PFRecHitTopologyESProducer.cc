...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  template<> uint32_t PFRecHitTopologyESProducer<ECAL>::GetNeighbourDetId(
    const uint32_t detId, const uint32_t direction, const CaloSubdetectorTopology& topo) {
  ...
  template<> uint32_t PFRecHitTopologyESProducer<HCAL>::GetNeighbourDetId(
    const uint32_t detId, const uint32_t direction, const CaloSubdetectorTopology& topo) {
    // desired order for PF: NORTH, SOUTH, EAST, WEST, NORTHEAST, SOUTHWEST, SOUTHEAST, NORTHWEST
    if(detId == 0)
      return 0;
