...
#define IsCpuSerial std::is_same_v<Queue, alpaka::QueueCpuBlocking>
...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  struct PFRecHitProducerKernelConstruct {
    ...
    ALPAKA_FN_ACC void operator()(const TAcc& acc,
      ...
      for (int32_t i : cms::alpakatools::elements_with_stride(acc, recHits.metadata().size())) {
        ...
        // Use atomic operation, when running in parallel. See comment in prepareEvent function below
        const uint32_t j = IsCpuSerial
                               ? num_pfRecHits[0]++
  ...
  void PFRecHitProducerKernel<CAL>::prepareEvent(Queue& queue, const uint32_t num_recHits) {
    ...
    // The first kernel requires global synchronisation, which is slow when using multiple blocks.
    // When using the serial CPU backend, there is no performance advantage in using
    // multiple blocks, but a significant penalty due to the synchronisation. Therefore, use
    // only a single block for the serial CPU backend.
    const uint32_t items = 64;
    const uint32_t groups = IsCpuSerial
                                ? 1
